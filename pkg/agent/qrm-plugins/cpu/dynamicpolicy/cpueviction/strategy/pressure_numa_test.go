...
func TestNumaCPUPressureEviction_GetTopEvictionPods(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			filterer, err := rules.NewFilter(metrics.DummyMetrics{}, candidatePodStats, tt.fields.numaPressureConfig.EnabledFilters)
			assert.NoError(t, err)
			scorer, err := rules.NewScorer(metrics.DummyMetrics{}, candidatePodStats, tt.fields.numaPressureConfig.EnabledScorers)
			assert.NoError(t, err)
			p := &NumaCPUPressureEviction{
