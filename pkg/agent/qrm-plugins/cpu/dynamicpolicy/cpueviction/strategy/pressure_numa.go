...
func NewCPUPressureUsageEviction(emitter metrics.MetricEmitter, metaServer *metaserver.MetaServer,
	...
	filterer, err := rules.NewFilter(emitter, evictParams, enabledFilters)
	if err != nil {
	...
	scorer, err := rules.NewScorer(emitter, evictParams, enabledScorers)
	if err != nil {
...
func (p *NumaCPUPressureEviction) UpdateEvictionParam(evictParams rules.EvictParams) {
	p.filterer.SetFilterParam(evictParams)
	p.scorer.SetScorerParam(evictParams)
}
...
func (p *NumaCPUPressureEviction) ThresholdMet(_ context.Context, req *pluginapi.GetThresholdMetRequest,
	...
	p.UpdateEvictionParam(p.evictParams)
...
func (p *NumaCPUPressureEviction) GetTopEvictionPods(ctx context.Context, request *pluginapi.GetTopEvictionPodsRequest,
	...
	if len(p.evictParams.NumaOverStats) == 0 {
		general.Warningf("no numa over stats")
		return &pluginapi.GetTopEvictionPodsResponse{}, nil
	}

	numaID := p.evictParams.NumaOverStats[0].NumaID
	numaUsageRatio := p.evictParams.NumaOverStats[0].AvgUsageRatio
	numaOverloadRatio := p.evictParams.NumaOverStats[0].OverloadRatio
...
func (p *NumaCPUPressureEviction) pickTopOverRatioNuma(metricName string, thresholds map[string]float64) (int, float64, float64, error) {
	...
	p.evictParams.NumaOverStats = numaOverRatios
...
func (p *NumaCPUPressureEviction) UpdateExtraEvictRules(name string, rules interface{}) {
	p.evictParams.ExtraRules[name] = rules
}
