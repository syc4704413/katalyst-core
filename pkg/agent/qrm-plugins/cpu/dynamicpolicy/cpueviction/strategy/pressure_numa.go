...
type NumaCPUPressureEviction struct {
	...
	candidatePodStats rules.EvictRules
...
func NewCPUPressureUsageEviction(emitter metrics.MetricEmitter, metaServer *metaserver.MetaServer,
	...
	filterer, err := rules.NewFilter(emitter, candidatePodStats, enabledFilters)
	if err != nil {
	...
	scorer, err := rules.NewScorer(emitter, candidatePodStats, enabledScorers)
	if err != nil {
	...
	return &NumaCPUPressureEviction{
		...
		candidatePodStats:  candidatePodStats,
		filterer:           filterer,
		scorer:             scorer,
	}, nil
}

func (p *NumaCPUPressureEviction) UpdateEvictionParam(candidatePodStats rules.EvictRules) {
	p.filterer.SetFilterParam(candidatePodStats)
	p.scorer.SetScorerParam(candidatePodStats)
}
...
func (p *NumaCPUPressureEviction) ThresholdMet(_ context.Context, req *pluginapi.GetThresholdMetRequest,
	...
	p.UpdateEvictionParam(p.candidatePodStats)
...
func (p *NumaCPUPressureEviction) GetTopEvictionPods(ctx context.Context, request *pluginapi.GetTopEvictionPodsRequest,
	...
	if len(p.candidatePodStats.NumaOverStats) == 0 {
		general.Warningf("no numa over stats")
		return &pluginapi.GetTopEvictionPodsResponse{}, nil
	}

	numaID := p.candidatePodStats.NumaOverStats[0].NumaID
	numaUsageRatio := p.candidatePodStats.NumaOverStats[0].AvgUsageRatio
	numaOverloadRatio := p.candidatePodStats.NumaOverStats[0].OverloadRatio
...
func (p *NumaCPUPressureEviction) pickTopOverRatioNuma(metricName string, thresholds map[string]float64) (int, float64, float64, error) {
	...
	p.candidatePodStats.NumaOverStats = numaOverRatios
...
func (p *NumaCPUPressureEviction) UpdateExtraEvictRules(name string, rules interface{}) {
	p.candidatePodStats.ExtraRules[name] = rules
}
