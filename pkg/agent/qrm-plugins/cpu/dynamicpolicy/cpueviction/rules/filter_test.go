...
func TestFilterer_Filter(t *testing.T) {
	...
	type fields struct {
		...
		filterParams   CandidatePodStats
	}
	type args struct {
		pods []*v1.Pod
	}
	...
	tests := []struct {
		...
		// 		enabledFilters: []string{OwnerRefFilterName, OverloadNumaFilterName},
		// 		filterParams: CandidatePodStats{
		// 			SkippedPodKinds:     "invalid OwnerRefFilter params",
		// 			NumaOverStats: "invalid OverlaodNumaFilter params",
		// 		},
		// 	},
		// 	args: args{
		// 		pods: []*v1.Pod{pod1, pod2, pod3},
...
func TestFilterer_SetFilter(t *testing.T) {
	t.Parallel()
	filterer, _ := NewFilter(metrics.DummyMetrics{}, CandidatePodStats{}, []string{"custom"})
	customFilter := func(pod *v1.Pod, candidatePodStats CandidatePodStats) bool {
...
func TestFilterer_SetFilterParam(t *testing.T) {
	t.Parallel()
	filterer, _ := NewFilter(metrics.DummyMetrics{}, CandidatePodStats{}, []string{OwnerRefFilterName})
	testParams := CandidatePodStats{
...
func TestNewFilter_InvalidFilter(t *testing.T) {
	t.Parallel()
	filterer, err := NewFilter(metrics.DummyMetrics{}, CandidatePodStats{}, []string{"invalid-filter"})
	assert.NoError(t, err)
	pod1 := makePod("pod1")
	var result []*v1.Pod
...
func TestFilterer_EmptyPodList(t *testing.T) {
	t.Parallel()
	filterer, _ := NewFilter(metrics.DummyMetrics{}, CandidatePodStats{
		SkippedPodKinds: []string{"DaemonSet"},
...
func TestOverloadNumaFilter_InvalidMetrics(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			params:       CandidatePodStats{NumaOverStats: []NumaOverStat{}},
			wantFiltered: true,
		},
		{
			name: "numa not found",
...
func TestRegisterFilter(t *testing.T) {
	t.Parallel()
	RegisterFilter("custom", func(pod *v1.Pod, candidatePodStats CandidatePodStats) bool {
		return true
	})
	filterer, _ := NewFilter(metrics.DummyMetrics{}, CandidatePodStats{}, []string{"custom"})
	if _, exists := filterer.filters["custom"]; !exists {
