...
func TestFilterer_SetFilterParam(t *testing.T) {
	t.Parallel()
	filterer, _ := NewFilter(metrics.DummyMetrics{}, EvictParams{}, []string{OwnerRefFilterName})
	testParams := EvictParams{
...
func TestNewFilter_InvalidFilter(t *testing.T) {
	t.Parallel()
	filterer, err := NewFilter(metrics.DummyMetrics{}, EvictParams{}, []string{"invalid-filter"})
	assert.NoError(t, err)
	pod1 := makePod("pod1")
	var result []*v1.Pod
...
func TestFilterer_EmptyPodList(t *testing.T) {
	t.Parallel()
	filterer, _ := NewFilter(metrics.DummyMetrics{}, EvictParams{
		SkippedPodKinds: []string{"DaemonSet"},
...
func TestOverloadNumaFilter_InvalidMetrics(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			params:       EvictParams{NumaOverStats: []NumaOverStat{}},
			wantFiltered: true,
		},
		{
			name: "numa not found",
