...
type FilterFunc func(pod *v1.Pod, evictRules EvictParams) bool

type Filterer struct {
	filterFuncs  map[string]FilterFunc
	filterParams EvictParams
...
func NewFilter(emitter metrics.MetricEmitter, evictRules EvictParams, enabledFilters []string) (*Filterer, error) {
...
func (f *Filterer) SetFilterParam(evictRules EvictParams) {
	f.filterParams = evictRules
}

func OwnerRefFilter(pod *v1.Pod, evictRules EvictParams) bool {
	if pod == nil {
		return false
	}
...
func OverloadNumaFilter(pod *v1.Pod, evictRules EvictParams) bool {
	if pod == nil {
		return false
	}
