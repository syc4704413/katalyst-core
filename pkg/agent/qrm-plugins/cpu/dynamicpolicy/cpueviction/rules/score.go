...
type ScoreFunc func(pod *CandidatePod, candidatePodStats CandidatePodStats) int

// Scorer manages a collection of scoring functions and their parameters
type Scorer struct {
	scorers      map[string]ScoreFunc
	scorerParams CandidatePodStats
...
func NewScorer(emitter metrics.MetricEmitter, candidatePodStats CandidatePodStats, enabledScorers []string) (*Scorer, error) {
	enabled := sets.NewString(enabledScorers...)
	scorers := make(map[string]ScoreFunc)

	for name := range enabled {
		if scorer, ok := allScores[name]; ok {
			scorers[name] = scorer
		} else {
...
func (s *Scorer) SetScorerParam(candidatePodStats CandidatePodStats) {
	s.scorerParams = candidatePodStats
}
...
func DeploymentEvictionFrequencyScorer(pod *CandidatePod, candidatePodStats CandidatePodStats) int {
	...
	deploymentEvictionFrequencyLimitStr := candidatePodStats.DeploymentEvictionFrequencyLimitStr
...
func UsageGapScorer(pod *CandidatePod, candidatePodStats CandidatePodStats) int {
	if pod == nil || pod.Pod == nil {
		return 0
	}
	numaOverStats := candidatePodStats.NumaOverStats
