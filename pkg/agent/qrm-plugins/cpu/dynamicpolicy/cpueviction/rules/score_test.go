...
func TestDeploymentEvictionFrequencyScorer(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			score := DeploymentEvictionFrequencyScorer(tt.pod, CandidatePodStats{})
...
func TestNewScorer_InvalidScorer(t *testing.T) {
	t.Parallel()
	scorer, err := NewScorer(metrics.DummyMetrics{}, CandidatePodStats{}, []string{"invalid-scorer"})
	assert.NoError(t, err)
	pod1 := makePod("pod1")
...
func TestScore_NilPods(t *testing.T) {
	t.Parallel()
	scorer, _ := NewScorer(metrics.DummyMetrics{}, CandidatePodStats{}, []string{UsageGapScorerName})
...
func TestScorer_EmptyScorers(t *testing.T) {
	t.Parallel()
	scorer, _ := NewScorer(metrics.DummyMetrics{}, CandidatePodStats{}, []string{})
...
func TestSetScorerParam(t *testing.T) {
	t.Parallel()
	scorer, _ := NewScorer(metrics.DummyMetrics{}, CandidatePodStats{}, []string{})
	testParam := CandidatePodStats{NumaOverStats: []NumaOverStat{{NumaID: 1}}}
...
func TestRegisterScorer(t *testing.T) {
	t.Parallel()
	customScore := 100
	customScorer := func(pod *CandidatePod, params CandidatePodStats) int {
		return customScore
	}
	RegisterScorer("custom", customScorer)
	scorer, _ := NewScorer(metrics.DummyMetrics{}, CandidatePodStats{}, []string{"custom"})
