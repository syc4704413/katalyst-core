...
func TestDeploymentEvictionFrequencyScorer(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			score := DeploymentEvictionFrequencyScorer(tt.pod, EvictParams{})
...
func TestNewScorer_InvalidScorer(t *testing.T) {
	t.Parallel()
	scorer, err := NewScorer(metrics.DummyMetrics{}, EvictParams{}, []string{"invalid-scorer"})
	assert.NoError(t, err)
	pod1 := makePod("pod1")
...
func TestScore_NilPods(t *testing.T) {
	t.Parallel()
	scorer, _ := NewScorer(metrics.DummyMetrics{}, EvictParams{}, []string{ScorerNameUsageGap})
...
func TestScorer_EmptyScorers(t *testing.T) {
	t.Parallel()
	scorer, _ := NewScorer(metrics.DummyMetrics{}, EvictParams{}, []string{})
...
func TestSetScorerParam(t *testing.T) {
	t.Parallel()
	scorer, _ := NewScorer(metrics.DummyMetrics{}, EvictParams{}, []string{})
	testParam := EvictParams{NumaOverStats: []NumaOverStat{{NumaID: 1}}}
