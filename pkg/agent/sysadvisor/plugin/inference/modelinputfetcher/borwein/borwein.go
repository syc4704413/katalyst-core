...
type (
	...
	GetContainerFeatureValueFunc func(timestamp int64, podUID string, metaServer *metaserver.MetaServer, metaReader metacache.MetaReader, conf *config.Configuration, extraConf interface{}) (map[string]interface{}, error)
)
...
func nativeGetContainerFeatureValue(timestamp int64, podUID string, metaServer *metaserver.MetaServer, metaReader metacache.MetaReader, conf *config.Configuration, extraConf interface{},
	...
	if err != nil {
		return nil, fmt.Errorf("GetPod for uid %s failed with error: %v", podUID, err)
	}
	...
	for _, container := range pod.Spec.Containers {
		...
		// Currently we only fetch cpu usage, we can extend this to a list of features.
		featureName := consts.MetricCPUUsageContainer
		data, err := metaReader.GetContainerMetric(podUID, container.Name, featureName)
		if err != nil {
			return nil, fmt.Errorf("GetContainerMetric for pod %s container %s feature %s failed with error: %v", podUID, container.Name, featureName, err)
		}

		containerFeatures[featureName] = data.Value
		podFeatures[container.Name] = containerFeatures
	}
...
func (bmif *BorweinModelInputFetcher) FetchModelInput(ctx context.Context, metaReader metacache.MetaReader,
	...
	for _, pod := range pods {
		...
		for _, container := range pod.Spec.Containers {
			containerInfo, err := getContainerFeatureValue(callTimestampInSec, string(pod.UID), metaServer, metaReader, bmif.conf, bmif.extraConf)
			if err != nil {
				return fmt.Errorf("get %v feature failed with error: %v", inferenceConsts.MetricDimensionContainer, err)
			}
			podFeature[container.Name] = containerInfo
		}
		ContainerInfo[string(pod.UID)] = podFeature
	}

	metaWriter.SetModelInput(inferenceConsts.MetricDimensionNode, NodeInfo)
	metaWriter.SetModelInput(inferenceConsts.MetricDimensionContainer, ContainerInfo)
