...
func generateTestConfiguration(t *testing.T) *config.Configuration {
	...
	stateFileDir, err := ioutil.TempDir("", "metacache-test")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	// defer os.RemoveAll(stateFileDir) // This should be called by the caller test function
	conf.GenericSysAdvisorConfiguration.StateFileDirectory = stateFileDir
	return conf
}
		...
		},
		{
			...
			args: args{
				conf: func() *config.Configuration {
					c := newConf()
					c.PolicyRama.EnableBorweinModelResultFetcher = false
					return c
				}(),
			},
			wantNil: true,
